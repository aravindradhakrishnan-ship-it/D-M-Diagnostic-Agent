...
def get_available_weeks(engine):
    ...
    # Return the previous 12 weeks excluding the most recent week (e.g., if current is W50, show W38–W49)
    if len(ordered) <= 1:
        return ordered
    without_current = ordered[:-1]
    if len(without_current) > 12:
        return without_current[-12:]
